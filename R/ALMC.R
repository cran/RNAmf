...
#' @usage ALMC_RNAmf(Xref = NULL, Xcand = NULL, fit, MC = FALSE, mc.sample = 100,
...
#' par(oldpar)}
#'
ALMC_RNAmf <- function(Xref = NULL, Xcand = NULL, fit, MC=FALSE, mc.sample = 100, cost = NULL, optim = TRUE, parallel = FALSE, ncore = 1, trace=TRUE) {
  t0 <- proc.time()
  ### check the object ###
  ...
  if (fit$level == 2) { # level 2
    ...
    ALMC.1 <- obj.ALC_level_1(Xnext, Xref = Xref, fit = fit, MC=MC, mc.sample = mc.sample, parallel = parallel, ncore = ncore)
    t3 <-proc.time()
    if(trace) cat("Calculating deduced variance for level 1:", (t3 - t2)[3], "seconds\n")

    ALMC.2 <- obj.ALC_level_2(Xnext, Xref = Xref, fit = fit, MC=MC, mc.sample = mc.sample, parallel = parallel, ncore = ncore)
    t4 <-proc.time()
    ...
    ALMC.1 <- obj.ALC_level_3_1(Xnext, Xref = Xref, fit = fit, MC=MC, mc.sample = mc.sample, parallel = parallel, ncore = ncore)
    t3 <-proc.time()
    if(trace) cat("Calculating deduced variance for level 1:", (t3 - t2)[3], "seconds\n")

    ALMC.2 <- obj.ALC_level_3_2(Xnext, Xref = Xref, fit = fit, MC=MC, mc.sample = mc.sample, parallel = parallel, ncore = ncore)
    t4 <-proc.time()
    if(trace) cat("Calculating deduced variance for level 2:", (t4 - t3)[3], "seconds\n")

    ALMC.3 <- obj.ALC_level_3_3(Xnext, Xref = Xref, fit = fit, MC=MC, mc.sample = mc.sample, parallel = parallel, ncore = ncore)
    t5 <-proc.time()
