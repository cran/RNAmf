...
#' @usage ALM_RNAmf(Xcand = NULL, fit, cost = NULL, optim = TRUE,
...
#' par(oldpar)}
#'
ALM_RNAmf <- function(Xcand = NULL, fit, cost = NULL, optim = TRUE, parallel = FALSE, ncore = 1, trace=TRUE) {
  t0 <- proc.time()
  ### check the object ###
  ...
  if (fit$level == 2) { # level 2
    ...
    t2 <- proc.time()
    if(trace) cat("Calculating the current variance at each level:", (t2 - t1)[3], "seconds\n")
    ...
    ALM <- list(ALM1 = optm.mat[1, ] / cost[1], ALM2 = optm.mat[2, ] / (cost[1] + cost[2]), ALM3 = optm.mat[3, ] / (cost[1] + cost[2] + cost[3]))
  } else {
  ...
  return(list(ALM = ALM, cost = cost, Xcand = Xcand, chosen = chosen, time = (proc.time() - t0)[3]))
}
